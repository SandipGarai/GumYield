...
Post-hoc test for Friedman test when comparing all pairs of treatments.

**Critical Difference (CD):**
$$CD = q_{\alpha,k,\infty} \times \sqrt{\frac{k(k+1)}{6n}}$$

Where $q_{\alpha,k,\infty}$ is the critical value from the studentized range distribution divided by $\sqrt{2}$.

**Decision Rule:**
$$|\bar{R}_i - \bar{R}_j| > CD \Rightarrow \text{Significant difference}$$

Where $\bar{R}_i$ is the mean rank of treatment $i$.
...
5. Dunn, O. J. (1964). Multiple comparisons using rank sums. *Technometrics*, 6(3), 241-252.
